...
function animateSkin(toggle) {
  let animate = localStorage.namemc_animate === 'true';
  if (toggle) {
    animate = !animate;
    localStorage.namemc_animate = animate.toString();
  }
  if (animate) {
...
function initializeSkin(id) {
  ...
  if (localStorage.namemc_animate === 'true') {
    animateSkin();
  } else {
    renderSkin();
  }
...
if (document.getElementById('nn_lb1') && !window.location.href.includes('pubcol')) {
  //setInterval(() => [...document.getElementsByTagName('ins')].forEach(e => e.remove()), 1000);
}
